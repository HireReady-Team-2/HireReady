...
def start_interview(req: StartSessionRequest, user=Depends(verify_token)):
    ...
    if req.resume_filename:
        db.save_message(session_id, "user", f"Resume attached: {req.resume_filename}")
...
async def start_with_resume(
    ...
    db.save_message(session_id, "user", f"Resume attached: {filename}")
    db.save_message(session_id, "assistant", ai_msg)
...
def chat(req: ChatRequest, user=Depends(verify_token)):
    ...
    if last_user_msg:
        db.save_message(req.session_id, "user", last_user_msg.get("content", ""))
...
async def parse_resume(file: UploadFile = File(...), user=Depends(verify_token)):
    filename = file.filename or ""
    if not filename.lower().endswith(".pdf"):
        raise HTTPException(status_code=400, detail="Only PDF files are supported")
...
# ─── System Prompt ─────────────────────────────────────────────────────────────
def build_system_prompt(track, interview_type, difficulty, num_questions, resume_text=None):
    resume_block = ""
    if resume_text:
        resume_block = f"\n\nCandidate resume (use to personalize questions): {resume_text}"
...
- Mixed: Alternate between technical and behavioral questions.{resume_block}
